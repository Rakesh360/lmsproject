...
''' THEME MODEL FOR APPLYING THEME '''
...
class SaveProblemAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            problem_id = save_problem(json_data, request, request.user , Problem, Topic)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if True:
                ...
                try:
                    adminstrator_obj = User.objects.get(username=request.user.username)
                    logger.info(
...
def SyncRemainingQuizTime(request):
    ...
    try:
        if is_user_authenticated(request) and request.user.role == "1":
            ...
            print(QuizStatus.objects.filter(quiz=quiz_config, applicant=applicant_obj).first().time_remaining)
            print('####')
